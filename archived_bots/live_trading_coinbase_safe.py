...
ESCALADA CONTROLADA:
✅ Fase 1: $20 USD completada (+0.09% P&L, 0 Kill Switch events)
🚀 Fase 2: $40 USD (escalada 2x con mismo risk profile)

RESTRICCIONES EXTREMAS DE SEGURIDAD:
1. Capital máximo: $40 USD (100% validado en Fase 1)
2. Position size: 10% del capital de trading ($4 USD en Fase 2)
...
class CoinbaseSafeTrading:
    ...
    def __init__(self, max_capital=40, paper_mode=True):
        self.max_capital = max_capital  # $40 USD (escalada desde $20)
        self.position_size_percent = 0.10  # 10% = $4 por trade en Fase 2
        ...
        print("\n" + "="*70)
        print(f"COINBASE SAFE TRADING - FASE 2 - {mode_str}")
        print("="*70)
        print(f"Fase 1 completada: $20 USD (+0.09% P&L, 0 Kill Switch)")
        print(f"Capital disponible total: $40.00")
        print(f"Capital de trading: ${self.max_capital} (escalada 2x)")
...
def main():
    print("\n" + "="*70)
    print("COINBASE SAFE TRADING - FASE 2 - Escalada Controlada")
    print("="*70)
    print("\n✅ FASE 1 COMPLETADA:")
    print("   - Capital: $20 USD")
    print("   - P&L: +$0.02 (+0.09%)")
    print("   - Max Drawdown: 0.02%")
    print("   - Kill Switch events: 0")
    print("   - Trades: 8")
    print("\n🚀 FASE 2 - ESCALADA:")
    print("   - Capital: $40 USD (2x)")
    print("   - Position size: $4 USD (2x)")
    print("   - Risk profile: MISMO (2%/3%/5% MDD)")
    print("\nCapital disponible: $40.00 USD")
    print("\nOPCIONES DE SEGURIDAD:")
    print("1. PAPER TRADING - Simulado con datos reales (RECOMENDADO)")
    print("2. LIVE TRADING - Real con $40 (ESCALADA VALIDADA)")
    ...
    try:
        ...
        if mode == "1":
            paper_mode = True
            max_capital = 40  # Simular con $40 (Fase 2)
            print("\n✅ Modo PAPER TRADING FASE 2 seleccionado (SIN RIESGO)")
            print("   Simulando escalada: $40 USD")
        elif mode == "2":
            confirm = input("\n⚠️  ¿Estás SEGURO de usar dinero real? (escribe 'SI ACEPTO FASE 2'): ").strip()
            if confirm != "SI ACEPTO FASE 2":
                print("\n[CANCELADO] Trading real cancelado por seguridad")
                return
            paper_mode = False
            max_capital = 40  # Escalada a $40 USD
            print("\n🔴 Modo LIVE TRADING FASE 2 activado (CON RIESGO REAL)")
            print("   Capital: $40 USD (escalada 2x validada)")
        else:
            print("\n[ERROR] Opción inválida")
            return
        
        duration_input = input("Duración en minutos (5-60): ").strip()
        if not duration_input:
            print("[ERROR] Debes ingresar una duración válida")
            return
